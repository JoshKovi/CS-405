...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  //Here we create a new string to check sql input string
  std::string newSQL;
  
  //While not the ideal solution of prepared statements or stored procedures, this protects against
  //or attacks, though this also limits the ability of the developer to build complex inquires using
  //this function, ideally input should be passed as parameters and the statement should be built in this function.
  //Another method would be regex checking each statement for Values that are equal i.e. 1=1, 'hack'='hack', etc.

  //If sql string contains or statement pass only end for error logging purposes 
  if (sql.find(" or ") <= sql.length()) { newSQL = sql.substr(sql.find("WHERE ") + 6, sql.length() - 1);}
  //Does not contain an or statement, attack vector would now have to replace end of string to dump database
  else { newSQL = sql; }
  ...
  char* error_message;
  if(sqlite3_exec(db, newSQL.c_str(), callback, &records, &error_message) != SQLITE_OK)
  {
      //Modified to display SQL query attempt, aka NAME='fred' OR *=*
    std::cout << "Data failed to be queried from USERS table. ERROR = " << error_message << " " << newSQL << std::endl;
    sqlite3_free(error_message);
    return false;
  }
