...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {
        auto step_pair = std::make_pair(result, increment);
        if (step_pair.first + step_pair.second > std::numeric_limits<T>::min() && step_pair.first + step_pair.second <= std::numeric_limits<T>::max() &&
            step_pair.first + step_pair.second >= result) { 
            //This checks to ensure addition is above min, below max, and is greater than or equal to result
            //Which ensures the increment is correct and has not exceeded the bounds of the data type
            result += increment;
        }
        else {
            //This return is used to flag to test function when an output is incorrect (excluding min() + 0 which could be handled before function call if necessary)
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        auto step_pair = std::make_pair(result, decrement);
        if (step_pair.first - step_pair.second <= result && step_pair.first >= step_pair.second
            && (step_pair.first - step_pair.second >= std::numeric_limits<T>::min() || step_pair.first - step_pair.second >=0) ) {
            //This checks that the decrement is less than the result, the result will not overflow after subtraction
            //and that the new result will not be less than the min value for the data type
            result -= decrement;
        }
        else {
            //This flags the test function that an overflow would have occurred (again excluding 0 as above).
...
template <typename T>
void printOutcome(std::string outputResult, T const& outputNum, T const& min_max) {
    if (outputNum == min_max) {
        //This signals the number had an overflow/underflow
        std::cout << outputResult << " occurance: True" << std::endl;
    }
    else {
...
void test_overflow()
{
    ...
    //These changes eliminate the false flagging of adding by 0 as an overflow condition
    if (increment == 0) {
        printOutcome("Overflow", start, (T)(start + 1));
    }
    else {
        result = add_numbers<T>(start, increment, steps);
        printOutcome("Overflow", result, std::numeric_limits<T>::min());
    }
...
void test_underflow()
{
    ...
    //These changes eliminate the false flagging of subtracting by 0 as an overflow condition
    if (decrement == 0) {
        printOutcome("Overflow", start, (T)(start - 1));
    }
    else {
        result = subtract_numbers<T>(start, decrement, steps);
        printOutcome("Underflow", result, std::numeric_limits<T>::max());
    }
